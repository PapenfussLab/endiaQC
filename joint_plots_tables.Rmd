...
First chunk loads 2 essential packages, and fetches the data from Figshare.
https://figshare.com/articles/ENDIA_microbiome_QC_OTUs_and_metadata/5401594
This is a 43MB tar file, so use with caution
```{r package_data}
...
#                      "ENDIA/ENDIA_QC")
download.file(url="https://ndownloader.figshare.com/files/9303850", destfile="OTUs.tar")
data_fp <- "OTU_data"
untar("OTUs.tar", exdir = data_fp, verbose = TRUE)
...
```{r load_biom_phylo_part_1}
## Set up filename variables
biom_fp <- file.path(data_fp, "WW_otu_table_mc3_allmeta.biom")
tree_fp <- file.path(data_fp, "WW_rep_set_nonchim_mc20.tre") 
map_fp <- file.path(data_fp, "mappingfullSampleInfoComplete.txt")
# localRdata_fp <- "local_WEHI"
...
# load(file.path(localRdata_fp, "rareWEHI72mc3_woReplace_v2.rdata"), verbose=TRUE)
...
prboxes + expand_limits(y=c(0,80)) + # hack to make y-axes match for InvSimpson between WEHI and BCM
...
## Consistent colours needed for 2 data sets, so hard-coding phylOrder. As BCM seqs gave 12 phyla,
## I am using that order, rather than the local WEHI seqs 10 phyla order
## Top 7 or 9 the WEHI and BCM sets are equal - top 8 they differ
...
## The 12 that have a proportion in some sample greater than 2% in BCM data makes 
## a good list - it includes the WEHI top 10 and BCM top 9. 
...
biom_fp <- file.path(data_fp, "WB_otu_table_mc3_allmeta.biom")
tree_fp <- file.path(data_fp, "WB_rep_set_nonchim_mc20.tre") 
localRdata_fp <- "local_BCM"
# if (!file.exists(localRdata_fp)) dir.create(localRdata_fp)
...
psSample_mc3 <- merge_samples(phyloData, "SampleNumber")
## This loses some or all the sample data 
## Remove column names that don't apply to merged categories. 
...
# save(psSample_mc3,
#      file=file.path(localRdata_fp, "phyloDataBySample_mc3") )
...
```{r alpha_rarefaction_extended_WB}
# localRdata_fp <- "local_BCM"
# load(file=file.path(localRdata_fp,
...
#       file=file.path(localRdata_fp, "rareBaylor72mc3_woReplace.rdata")
...
ggplot(
  ...
  geom_vline(xintercept = min(sample_sums(psSample_mc3))) + 
  labs(title="BCM richness, 72 samples, mc3, 8 x downsampled without replacement",
       y="Mean alpha diversity") + 
  facet_wrap(facets = ~ Measure,
             scales = 'free_y')
...
```{r alpha_richness_plots_WB}
## Make the alpha diversity and library size plots. 
## Use all OTUs (min count=3), 216 samples
## 2 measures side-by-side, downsampled data

# load(file.path(localRdata_fp, "rareBaylor72mc3_woReplace.rdata"), verbose=TRUE)
...
prboxes + labs(
       x = 'Participant', 
       y = 'Alpha diversity', 
  title="72 Samples (12 per box), BCM data, mean of 10 subsamples")
## Figure 4B

rm(rarefaction_baylor_means, alpha_sub_ObsSimp)

## Library sizes. As bar chart for each sample at stool level (72)
# load(file.path(localRdata_fp, "phyloDataBySample_mc3.rdata"), verbose=TRUE) 
libDir <- file.path(baseDir, "analysis/local/sample_sizes")

libsize_df <- data.frame(libsize=sample_sums(psSample_mc3),
                         sample_data(psSample_mc3),
                         sXm=paste0(sample_data(psSample_mc3)$Stool, 
                                    sample_data(psSample_mc3)$Method)
)

libsize_df$aliquot <- as.factor(
  sapply(strsplit(as.character(sample_data(psSample_mc3)$SampleNumLvl2), '.', fixed = TRUE), "[[", 2)
)
...
## I want to summarize the taxa-table of the 26 OTUs diff between A and D for BCM sequences
...
End of WB work
```{r cleanup_W_BCM}
rm(phyloData, psSample_mc3, otuBySampleBy3, otuBySample, l6Std, psL6prop, 
   l6Std_72, psL6prop_72)
```

Sequencing centre comparison - joint QIIME run
WB_comparison_DESeq.R - load data section
```{r WB_comparison_DESeq_load_data}
localRdata_fp <- "local_BCM_1wellWEHI"
### Load WEHI-BCM data - new OTU table with sequences of WEHI and BCM clustered together ###
...
WBotu_mc20<- import_biom(file.path(data_fp,"Wcombined_otu_table_mc20_allmeta.biom"),
...
phylotree<-read_tree(file.path(data_fp,
                               "Wcombined_rep_set_nonchim_mc20.tre"))
...
```{r WB_comparison_DESeq_DESeq}
### Differential abundance Hypothesis testing between WEHI and BCM data using DESeq2 - min count 20 ###
gm_mean = function(x, na.rm=TRUE){
  exp(sum(log(x[x > 0]), na.rm=na.rm) / length(x))
}
...
SeqCentreResO_mc20Fil$maxPropBCM <- apply(otu_table(OrderPropB), 1, max)[
  rownames(SeqCentreResO_mc20Fil)]
...
SeqCentreResP_mc20Fil$maxPropBCM <- apply(otu_table(phylaPropB), 1, max)[
  rownames(SeqCentreResP_mc20Fil)]
...
```{r WB_comparison_Beta_diversity}
...
```{r cleanup_WBcombined}
rm(WBotu_mc20_Complete, localRdata_fp)
```


Baylor College Medicine original biom work
```{r load_biom_BCM_orig}
## Reading the rarefied biom file produced by BCM from their bioinformatics pipeline
## "PennoM_P2_StrictMerge.biom"
## and reproducing plots for it
biom_fp <- file.path(data_fp, "BCM_StrictMerge.biom")
map_fp <- file.path(data_fp, "BCM_Mapping_SampleSheet.txt")
tree_fp <- file.path(data_fp, "silva123_V4.tre") 

# localRdata_fp <- "BCM_data"
